...
| **Announcement** | A one-to-many broadcast from an instructor to all enrolled course members. Has a `title`, rich text `body` (sanitized), `is_published` and `pinned` flags. Ordered by pinned-first then newest. Creating a published announcement generates Notifications for all active course members. |
| **Notification** | An in-app notification delivered to a user. Has `event_type` (announcement, assignment_posted, grade_posted, etc.), `title`, `body`, optional `course` FK, `link` (relative URL), and `is_read` flag. Scoped to user (account-scoped through User). Delivered via 60-second polling (V1). |
