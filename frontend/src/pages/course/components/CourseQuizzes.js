...
function CourseQuizzes({ assignments, courseId, isInstructor, onAssignmentsChange }) {
  ...
  const toggleSelectAll = () => {
    if (selected.size === sortedQuizzes.length) {
      setSelected(new Set());
    } else {
      setSelected(new Set(sortedQuizzes.map(q => q.id)));
    }
  };
      ...
      {isInstructor && (
        <div className="assignment-bulk-bar">
          <label className="select-all-label">
            <input
              type="checkbox"
              checked={selected.size === sortedQuizzes.length}
